...
class Decoder(nn.Module):
    ...
    def __init__(self, use_interaction):
        ...
        if use_interaction:
            self.cell = nn.LSTMCell(input_size=128, hidden_size=384)
            self.plan_input = nn.Linear(3, 128)
            self.state_input = nn.Linear(3, 128)
        else:
            self.cell = nn.LSTMCell(input_size=3, hidden_size=384)
    ...
    def forward(self, init_hidden, plan, gate, init_state):
        ...
        cell_state = torch.zeros_like(init_hidden)

        # Iterate over the 30 time steps to predict the trajectory
        for t in range(30):
            if self.use_interaction:
                # Use interaction-aware input, which includes the plan and state 
            ...
            # Pass the input through the LSTM cell and update the hidden and cell states
            hidden, cell_state = self.cell(input, (hidden, cell_state))
            state = self.decode(hidden) + state[:, :3]
            output.append(state)

        # Stack the output to form a tensor
